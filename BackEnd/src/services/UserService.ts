...
import { loginSchema, userOptionalSchema, userSchema } from "@/common/http/schemas/userSchema";
...
export class UserService {
    ...
    async login(args: Partial<User>) {
        ...
        const token = jwt.sign({id: user.id}, env.JWT_PASS ?? 'secret', {expiresIn: '8h'});
    ...
    async update(args: Partial<User>, currentUser: Partial<User>) {
        ...
        let emailExists = _user.data.email && await userRepository.findOneBy({email: _user.data.email});
        ...
        const password = _user.data.password ? _user.data.password : oldUser.password

        const hashPassword = password ? await bcrypt.hash(password, 10) : oldUser.password;

        await userRepository.update(id, {name, email, password: hashPassword})
    }
